...
- **Distro**: [e.g., Fedora 44, Ubuntu 26.04, Debian 13]
- **Kernel**: [e.g., 6.18.9-200.fc43.x86_64]
- **Bash Version**: [e.g., 5.2.37]
- **Desktop Environment**: [e.g., GNOME 48, KDE Plasma 6]
- **Script Version**: [e.g., v3.7.1]
- **Flags Used**: [e.g., --ai, --json, --verbose, --cis-l1, --skip btprivacy]
- **ENV Vars**: [e.g., NOID_AIDE_LIVE=1, NOID_RPM_BASELINE_INIT=1]
- **Score / assessed coverage**: [e.g., 78 / 86]
- **VM, container, or physical host**:
...
- [ ] I removed usernames, hostnames, addresses, paths, tokens, and unrelated findings

Do not attach an unreviewed `bash -x` trace. Root-level traces can contain
sensitive configuration and secret-adjacent values. Prefer the smallest
sanitized fixture that reproduces the parser or classification error.
