...
The terminal and AI summary show **Scan completed**: the number of distinct
canonical sections run, divided by 42. A full run displays **100% (42/42
sections)**. An explicitly skipped section is not counted; unavailable individual
checks remain visible in the findings. Completing the scan does not mean every
control passed or every optional observation was available.

The AI summary uses the same scan-completion measure and lists sections with
incomplete scoring evidence and selected concrete evidence limits in words.
The separate numerical coverage metric remains available to JSON consumers and CI.

The `section-risk-v1` assessment model separately retains two values:

- **Desktop posture score**: the result within the assessed security and
  privacy scope, from 0 to 100.
- **Risk-weight coverage** (JSON and CI): how much of the fixed model
  produced an assessed result, from 0% to 100%. This is assessment scope, not
  scan progress.
