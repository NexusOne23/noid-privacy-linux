...
A correctness update within version 3.7.2. The audit keeps its non-remediating
default and its 42 risk-weighted sections. Corrections cover privileged PATH
and integrity evidence, DNS and firewall observations, failed inventory
queries, GNOME retention, and a stable local-account view during each audit.

### Highlights

- **AIDE candidates are no longer trusted as a baseline.** `aide.db.new*` files are
  reported only as pending candidates: they cannot produce a database PASS or make a
  prior scheduled check look stale. Accepting an active baseline stays an explicit
  operator decision, and a clean scheduled result earns PASS only when a nonempty
  active database is proven unchanged since that check.
- **`/usr/local` is used only where it is provably root-owned.** `/usr/local/bin` and
  `/usr/local/sbin` remain available on the supported NoID Workstation layout, but only
  after the directory, each top-level command candidate and every resolved symlink
  target are confirmed root-owned and not group- or world-writable.
- **The egress-identity comparison is deterministic.** Both observations now use IPv4
  explicitly, so a dual-stack VPN host can no longer pair an IPv4 DNS identity with an
  IPv6 HTTPS identity and warn about a DNS-routing mismatch that does not exist. Public
  addresses remain redacted.
- **The local account database is read once per audit.** The main shell snapshots
  `/etc/passwd` before any section runs; every account check inherits that readonly
  snapshot, so sections cannot observe different local-account states during one run.
  Evidence-capture timing changes, no grading rule does.

### Fixed — stable local-account evidence

- The main shell snapshots `/etc/passwd` exactly once before any audit section
  runs. Process substitutions and pipelines inherit that readonly snapshot, so
  account sections cannot observe different local-account states during one
  audit. Structural and functional regressions cover both the single
  production load and repeated production-shaped process substitutions.
- Singular history findings now use singular nouns in both the interactive
  report and the `--ai` warning summary.

### Fixed — boot, kernel and module evidence

- Preserve failed Secure Boot queries as unknown; scope display-manager and suspend-lock findings to their actual evidence.
- Validate kernel taint and IMA counter reads, retain new/unknown taint bits, and separate recorded measurements from appraisal or coverage claims.
- Verify native module load plans and current driver presence; comments and blacklists no longer masquerade as load suppression.
- Rescue commands are identified without inventing a password challenge.
- Basic and optional sysctl checks accept only successful integer reads within
  the shell's numeric range. Empty, failed, partial or overflowing values cannot
  become hardened zeroes or shell arithmetic errors. SysRq details reuse the
  successfully assessed value, and unreadable keys remain explicitly unassessed.
- IPv4 forwarding is graded from a successful exact `0` or `1` query. Failed,
  partial or unexpected output no longer becomes a disabled-forwarding PASS.
  VPN presence alone does not certify that forwarding is needed; detected VM
  networking remains context for reviewing the firewall's forwarding policy.
- The exact NoID permission policy recognizes retained kernel `System.map`
  files with root-only `0600` access. It still rejects content/capability drift,
  wrong ownership, unsafe paths and changed file types. The optional RPM
  reference-profile notice now distinguishes an unconfigured comparison from
  a missing package; no trust baseline is generated or accepted by the audit.

### Fixed — telemetry and crash-reporting evidence

- Ubuntu crash-service query failures no longer imply disabled collection or
  inactive upload triggers. Pending Ubuntu Report files may contain opt-outs;
  custom cache paths and transmission stay unassessed. Debian popularity-contest
  inventory distinguishes package-query errors and reports its main preference
  without treating it as the effective sourced shell configuration.
- ABRT service inventory includes the main `abrtd.service` and preserves query
  failures. Its system reporting preference comes from the native read-only
  command; local collection and desktop-user preferences stay distinct. Removed
  the unsupported Snap `experimental.telemetry` probe and its misleading verdicts.
- NetworkManager connectivity findings use its runtime D-Bus properties;
  an earlier disabling snippet can no longer hide a later override. Reading
  the setting neither starts the service nor requests a connectivity probe.
  DNF countme checks resolve repository overrides through native DNF5 or DNF4
  configuration readers, distinguish enabled repositories and HTTP mirror
  discovery, and preserve failed-query uncertainty without exposing URLs.
- GNOME indexer checks distinguish failed user-manager queries from confirmed
  inactivity and include verified standalone Tracker/LocalSearch processes.
  Missing or partial account/unit evidence cannot produce an inactivity PASS.
  GNOME privacy booleans also require successful reads; unsupported schemas,
  missing session access and partial query failures remain visibly unassessed.

### Fixed — desktop, audio and sandbox evidence

- NTP and Bluetooth queries preserve failed or incomplete native responses.
  NTS mode alone no longer counts as established authentication keys, and
  Bluetooth masks no longer hide an already running service. Radio-block
  findings require complete explicit rfkill state.
- Audio findings require successful, recognizable native responses. An unavailable
  default source no longer implies missing microphone hardware; mute evidence is
  scoped to that source. Network audio inventory reads running server modules and
  actual TCP listeners instead of treating configuration examples as live exposure.
- Flatpak permission checks now use complete app refs and installation identity,
  with native merged overrides for each inspected local login account and root.
  Read-only broad filesystem grants, unfiltered buses and Flatpak host-command
  access are recognized. Failed or diagnostic-bearing inventories cannot produce
  an all-clear; empty successful permission output remains valid. Environment
  values never supply findings. The bounded screen names its configuration and
  parsing limits instead of claiming universal sandbox safety.

### Fixed — certificate and SSH evidence

- Certificate expiry now requires a successful native date parse; empty,
  unreadable and malformed files no longer appear expired. CA inventory uses
  the explicit anchor filter and preserves query failures. The bundle fallback
  reports PEM blocks rather than inferred trusted identities. File selection
  preserves literal names, deduplicates aliases and reports parsing limits;
  first-certificate scope and active-service uncertainty remain explicit.
- SSH certificates now receive the same subject-key size checks as bare keys;
  an undersized RSA certificate can no longer bypass the RSA rule. Key files
  are read line by line through the native parser, with unreadable or unparsed
  content explicitly unassessed. Unknown key types no longer receive a guessed
  PASS. Conventional files are enumerated from the local-account snapshot,
  including custom and service-account homes. The certificate section counts
  parsed records instead of treating every authorized_keys line as a key;
  certificate trust, key options and actual access grants remain separate.
- SSH activation checks include runtime-enabled units and verified OpenSSH
  processes; query errors cannot establish absence. Configuration checks share
  one successful native `sshd -G` response (`-T` compatibility fallback), with
  no text-grep/default substitution after parser errors. Global configuration,
  connection-specific `Match` rules and daemon launch overrides are explicitly
  distinguished. Zero login grace time is unlimited, access-list patterns are
  inventory, and algorithm exclusions are resolved before the legacy screen.
  Algorithm regression fixtures now invoke the production classifier.

### Fixed — process, service and listener identification

- Named-process candidates are verified against the native executable or a
  supported CPython script operand. Arguments, rewritten `argv[0]`, regex
  punctuation and truncated kernel names cannot substitute for that check.
  Failed reads remain unassessed in WS-Discovery, clipboard, KDE indexer and
  audio consumers. WS-Discovery output separates unit/process/option inventory
  from actual traffic, parentage and firewall reachability.
- Service groups check successful systemd property responses for all known
  aliases and activation units. Query failures cannot become service-off PASS;
  a masked alias cannot hide an enabled sibling. Runtime-only masks/enablement
  are explicit, and failed-unit/timer queries discard partial results. Actual
  environment-specific unit failures remain visible as operational inventory.
- Parse remote-desktop listener endpoints exactly, including when the general ports section is skipped.

### Fixed — per-link network-policy evidence

- Read active per-link network policy: independent IPv4/IPv6 DHCP hostname settings, per-interface temporary-address settings, and resolved mDNS runtime state. Keep MAC generation, profile defaults, and actual packet behavior distinct.

### Fixed — VPN, kill-switch and LAN observations

- Dummy routing kill-switches are now checked by mechanism: both address
  families, the complete supported routing program, winning fallback metrics,
  link state and forwarding hazards. Explicit host-route exceptions remain
  visible. Native isolated TCP tests cover tunnel removal, a losing guard
  metric, and route exceptions. The audit never disconnects the host VPN.
- nftables boot persistence requires persistent enablement of the actual
  managing service; an active firewalld alone cannot establish boot enablement.
- Dummy interfaces are reported as routing helpers and cannot gain VPN status
  from a name or default route. WireGuard is recognized by native link type with
  any interface name. Bridged TUN/TAP detection uses the same network namespace,
  and failed/partial/mismatched type queries cannot earn VPN confirmation.
- VPN link activity requires a successful, matching administrative-UP record.
  A successful empty `ip link show up dev` response means DOWN; query failures,
  malformed records and partial inventories stay unassessed. The same guard
  protects route and resolver-link checks. UP alone does not attest an
  authenticated peer or actual tunnel traffic.
- Kill-switch detection no longer awards PASS from policy-routing selectors,
  interface DROP text or duplicate-rule counts. Native isolated tests reproduce
  routing fallback after tunnel removal and reachable TCP traffic despite
  matching DROP rules. Unsupported mechanisms remain checked inventories;
  failures discard partial output, and unverified protection with an active VPN
  marks section evidence incomplete. No VPN remains a supported optional policy.
- LAN ICMP probes no longer award a blocked-network PASS for missing replies,
  command errors or replies associated with a VPN route. A live isolated peer
  remained TCP-reachable while dropping only ICMP, demonstrating why these
  observations cannot establish LAN isolation. Probe failures retain their
  return code and the existing offline/skip controls remain effective.

### Fixed — deterministic egress-identity comparison

- Resolver-based VPN ownership requires an exact DNS-address token and a
  successful status query. A longer address with the same suffix, or partial
  output from a failed query, can no longer establish positive VPN evidence.
  DNS-over-TLS server names and the confirmed active-route path remain
  supported.
- IP response validation rejects leading-zero IPv4 octets and dangling IPv6
  colons. These malformed strings previously passed the syntax check and
  could also pass public-address classification, supplying invalid evidence
  to the egress comparison. Valid decimal octets and IPv6 compression remain
  accepted.
- The direct-DNS vs. HTTPS egress comparison now explicitly uses IPv4 for both
  observations. The current authoritative endpoint exposes the identity as an
  A record through an IPv4-only authority; on dual-stack VPN hosts, curl's
  Happy-Eyeballs preference could otherwise pair that IPv4 DNS identity with
  an IPv6 HTTPS identity of the same tunnel and warn about a DNS-routing
  mismatch that did not exist. Public addresses remain redacted.

### Fixed — firewall and socket evidence

- Listener reachability requires successful firewalld zone, policy, and
  direct-rule queries. Failed or partial global responses remain unassessed
  throughout the audit instead of being cached as an empty, blocking policy.
  The native `no zone` result still falls back to the default zone.
- Failed socket queries no longer produce PASS findings for absent TCP/UDP
  listeners, outbound peer ports, or raw sockets. Partial failed output is
  discarded; successful empty and populated inventories retain their findings.
  Peer-direction classification reuses the checked TCP listener snapshot.

### Fixed — AIDE baseline and drift evidence

- Scheduled AIDE drift details now require a unique private report carrying
  the selected systemd invocation ID. Delayed report creation no longer loses
  the correct file or substitutes an overlapping shared log. Journal fallback
  binds both unit and invocation and discards failed reads; missing or ambiguous
  detail evidence never suppresses the recorded drift warning.
- AIDE drift details preserve Added/Removed categories and complete paths
  containing colons. The renderer previously relabeled parsed report entries
  as Changed and could discard the start of a filename after another colon.
- AIDE `aide.db.new*` files are now reported only as pending candidates. They
  cannot create a database PASS or make a prior scheduled check appear stale;
  accepting an active baseline remains an explicit operator decision. A clean
  scheduled result earns a historical PASS only with verified completion and
  nonempty active-database metadata predating the check's start; current file
  state is explicitly not rechecked by this status lookup.
- AIDE drift remains WARN after database metadata changes: timestamps cannot
  establish operator acceptance or resolve an integrity finding. Scheduled
  evidence is captured per service in one checked query, selects the newest
  recorded invocation, distinguishes normal exits from signals, and rejects
  running, partial, and inconsistent execution records. Changes to database
  mtime or ctime during a check cannot support a clean baseline finding.

### Fixed — RPM verification evidence

- RPM verification retains bytecode and all `__pycache__` discrepancies for
  grading and state fingerprinting. These paths can contain executable code;
  their names no longer justify a clean finding for changed content, security
  metadata, or missing files. Timestamp-only differences remain separate.
- RPM stderr diagnostics are preserved, and exit 1 without verification
  evidence stays incomplete. A database read error previously lost its only
  diagnostic and could be reported as a clean empty inventory.
- RPM integrity fingerprints preserve trailing newlines in symlink targets.
  A target change consisting only of these bytes previously disappeared in
  shell command substitution and could match the old expected-state record.

### Fixed — privileged PATH and /usr/local admission

- Privileged PATH admission now checks every directory and symlink in lookup
  order before following it. Resolving only the final target could previously
  accept a root-owned command alias through a user-replaceable intermediate
  link. Trusted absolute and relative links remain supported; mutable hops,
  invalid directory traversal, and excessive link chains are rejected.
- `/usr/local/bin` and `/usr/local/sbin` remain available to the audit on the
  supported NoID Workstation layout, but only after the directory, each
  top-level command candidate, and every resolved symlink target are confirmed
  root-owned and not group- or world-writable.

### Fixed — browser, clipboard and report-content evidence

- Parse Firefox profile preferences as data without executing JavaScript; distinguish cookie blocking from partitioning.
- Correct clipboard persistence, update-reminder activation, AIDE hash-reference and key-agent inventory claims.
- Withhold cron/history command text from reports.

### Fixed — GNOME recent-file retention evidence

- The native signed `recent-files-max-age=-1` value is reported as indefinite
  retention. Removing the minus sign previously produced a false one-day PASS.
  Failed queries, malformed values, and unsupported negative durations remain
  unassessed; successful zero and finite durations retain their grading.

### Fixed — reporting and AI hand-off

- The AI summary uses the same scan-completion percentage as the terminal.
  Incomplete scoring evidence stays visible by section and concrete finding;
  numerical assessment coverage remains available in JSON and CI.
- The terminal summary reports actual section execution as `Scan completed:
  100% (42/42 sections)` for a full run. Weighted assessment coverage remains
  available to JSON/AI consumers and CI, independently of scan progress.
- The AI hand-off includes selected INFO evidence limits and names unassessed
  weighted sections. Unverified kill-switch protection, an unestablished NoID
  AIDE baseline and unclassified substantive RPM discrepancies no longer vanish
  behind a FAIL/WARN-only summary. Severity, counters and scoring are unchanged;
  coverage is explicitly section-level, and missing evidence does not authorize
  baseline acceptance. Finding text remains sanitized, untrusted quoted data.

### Documentation

- Contribution-guide navigation uses GitHub's actual heading fragments,
  including the emoji-derived prefixes.
- Release validation separates historical VM results from checks of the
  current source bytes and acknowledges that severity corrections can change
  findings, score, and coverage.
- Quick Start, tag download and tag clone commands name one checksum, so a
  repository checkout and a tag download verify identically.
- The README leads with what the audit does and keeps the operator detail in
  `Docs/CHECKS.md`: the root rationale is two sentences, and the stateful
  opt-ins are referenced instead of repeated above the Quick Start command.

### Changed — continuous integration

- A release tag no longer triggers a second identical CI run. Tags point at a
  commit the branch run already validated, so the workflow reacts to branch
  pushes and pull requests only.
- The style gate runs a digest-pinned ShellCheck 0.11.0 image instead of the
  runner image's distribution package. The gate previously changed strictness
  with that image: the shipped 0.9.0 reports called functions as unreachable
  under SC2317 and flags documented optional parameters under SC2119/SC2120,
  while the version this source is developed and validated against reports no
  findings. Pinning the analyzer keeps the verdict reproducible.

### Tests

- PASS-emitter and aggregation tests execute the production helpers,
  including section counters and JSON findings. The previous copied
  implementations still passed when their production functions were replaced
  with deliberate failures; mutation checks now detect those regressions.
---
...
### Fixed — dual-stack egress-identity comparison

- The direct-DNS vs. HTTPS egress comparison now stays within one address
  family: the HTTPS observation is forced onto the family that answered the
  authoritative DNS observation. On dual-stack VPN hosts, curl's
  Happy-Eyeballs preference could previously pair an IPv4 DNS identity with
  an IPv6 HTTPS identity of the same tunnel and warn about a DNS-routing
  mismatch that did not exist. Finding texts now state the single-family
  scope explicitly.
...
### Fixed — ARP math, cosmetic polish and Audit Round-2 (detection + display)
...
### Fixed — Live-Audit self-review, 10 detection-accuracy fixes
...
### Fixed — ausearch hang and the Anaconda WebUI RPM-verify exclusion
...
### Fixed — hardened-distro Live-VM false positives (5 fixes)
...
- v3.6.1: 93% WELL-HARDENED (255 PASS, 3 FAIL, 13 WARN)
...
### Fixed — Live-ISO false positives, reporting quality, engineering/self/live audit and polish (64 fixes)
...
### Changed — posture communication, detection depth, engineering discipline and compliance
...
### Fixed — Phase-8 audit closure and post-audit polish (DE dispatcher, cross-distro, ShellCheck-clean, 15 final fixes)
...
### Fixed — post-release fixup after v3.4.0 user testing
...
### Fixed — critical severity, False-FAIL elimination
...
### Changed — style and presentation
...
### Added — 32 checks from the Lynis comparison
...
### Added — 5 checks closing the final Lynis gaps
...
### Fixed — false positives on real-world hardened systems
...
- **`kernel.unprivileged_bpf_disabled = 2` false positive**: Values `1` and `2` both disable unprivileged BPF. Value `1` cannot be cleared in the running kernel; value `2` remains administrator-reversible. Both are accepted for this control.
...
**Quality & Correctness Release.** 27 issues fixed from comprehensive code review. Zero new features — pure quality improvement.
...
### Fixed — desktop intelligence, false-positive reduction
...
### Added — multi-distro support (Arch and openSUSE)
...
### Added — Debian/Ubuntu feature parity
...
### Fixed — additional post-review bugfixes
...
### Fixed — low severity and nitpicks
...
**Major Release — Privacy & Security Audit for Linux Desktops.** Complete repositioning: From server security tool to the first desktop privacy & security auditor for Linux.
...
#### Desktop Session Security (Section 39)
...
#### Webcam & Audio Privacy (Section 40)
