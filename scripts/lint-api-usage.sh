...
# `chage -l` output is translated. grep "Maximum" silently misses the
# localized form → silent FALSE PASS on non-English systems.
...
# columns are all locale-translatable on non-English systems.
...
# Self-audit found 5 such sites parsing `^Mem:`/`^Swap:` headers that a
# non-English locale translates — silently empty MEM_TOTAL etc.
