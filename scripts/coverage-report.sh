#!/usr/bin/env bash
#
# Coverage-Report Generator (v3.7.0)
#
# Parses Docs/CIS_RHEL9_MAPPING.md and generates a summary of how many
# CIS L1 / L2 / STIG controls NoID Privacy maps to. Used by the main script when
...
# Extract mapping rows: lines starting with `| 0` (NoID Privacy section number) and
...
case "$MODE" in
  ...
  "")
    ...
    print_section "DISA STIG RHEL 9:" "$STIG_COUNT" "$STIG_TOTAL"
    echo ""
    echo "Total NoID Privacy checks mapped: $TOTAL_ROWS"
    echo ""
    echo "Note: NoID Privacy is a hardening posture audit, not a compliance scanner."
