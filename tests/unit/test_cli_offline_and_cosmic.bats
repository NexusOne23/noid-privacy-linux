...
@test "version flag is available without root and exits successfully" {
  ...
  [[ "$status" -eq 0 ]]
  [[ "$output" == "NoID Privacy for Linux 3.7.2" ]]
}
...
@test "no-VPN posture and ordinary IPv6 are not unconditional adverse findings" {
  ...
  grep -q '_emit_info "No administratively UP VPN interface confirmed (valid no-VPN posture' "$SCRIPT"
}
...
@test "optional telemetry tools require effective evidence before adverse grading" {
  grep -q 'Snap client installed; snap and store data collection is not assessed' "$SCRIPT"
  grep -q 'ABRT system services running or transitioning:.*automatic upload not proven' "$SCRIPT"
