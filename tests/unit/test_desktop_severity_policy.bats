...
@test "disabled global IPv6 does not claim that every physical IPv6 scope is absent" {
  grep -q 'active hardware-backed interfaces (temporary addressing not applicable there)' "$SCRIPT"
...
@test "VPN full-tunnel evidence checks every route table and only confirmed VPN devices" {
  ...
  ip() {
    if [[ "$1" == "-4" && "$2" == "route" ]]; then
      ...
      printf '%s\n' '7: wg0: <UP,LOWER_UP> mtu 1420 state UNKNOWN'
    else
      return 1
    fi
  }
...
@test "unreadable optional runtime evidence remains unassessed" {
  ...
  dhcp_block=$(sed -n '/^_nm_dhcp_profile_audit() {/,/^}/p' "$SCRIPT")
  ...
  [[ "$dhcp_block" == *'active profile unavailable (unassessed)'* ]]
...
@test "per-link IPv6 policy does not grade the default template as runtime state" {
  netprivacy_block=$(sed -n '/^check_network_privacy() {/,/^}/p' "$SCRIPT")
  [[ "$netprivacy_block" == *'no active hardware-backed interface found'* ]]
  [[ "$netprivacy_block" != *'net.ipv6.conf.default.use_tempaddr'* ]]
}

@test "IPv6 temporary-address finding describes linkability rather than identity proof" {
  netprivacy_block=$(sed -n '/^check_network_privacy() {/,/^}/p' "$SCRIPT")
  [[ "$netprivacy_block" == *'review stable-address linkability where global IPv6 is used'* ]]
...
@test "Ubuntu crash telemetry separates capture consent trigger and upload evidence" {
  ...
  [[ "$telemetry_block" == *'Apport service active or transitioning'* ]]
