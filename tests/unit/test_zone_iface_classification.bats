...
# — confirmed VPN tunnels (_iface_is_vpn) OR VM/container virtual interfaces
...
@test "F-387: _VIRT_IFACE_REGEX matches VM/container interface families" {
  for ifc in virbr0 virbr0-nic vnet0 vnet12 docker0 br-1a2b3c veth9f0e podman0 cni-podman0 lxcbr0 lxdbr0 tap0 tap-noid-test; do
...
@test "F-387: firewall zone loop uses semantic VPN plus virtual classifiers" {
  # shellcheck disable=SC2016  # literal source-code pattern
  grep -q '! _iface_is_vpn "$_iface"' "$SCRIPT"
  # Literal source-code pattern.
  # shellcheck disable=SC2016
  grep -q 'grep -qE "$_VIRT_IFACE_REGEX"' "$SCRIPT"
}
