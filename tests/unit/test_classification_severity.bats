...
setup() {
  ...
  source <(awk '
    /^(_active_chrony_unit|_chrony_source_counts|_chrony_source_quality_state)\(\) \{/ { in_function=1 }
    in_function { print }
...
@test "F-273: pam-no-nullok contains no nullok keyword" {
  run grep -qE '^[[:space:]]*[^#[:space:]].*nullok' "$FIXTURE_DIR/pam-no-nullok.txt"
  [[ "$status" -ne 0 ]]
}
...
@test "F-275: counter increment uses safe \$((var+1)) form" {
  ...
  grep -q '_LOCKED_ACCOUNTS=\$((_LOCKED_ACCOUNTS + 1))' <<< "$block"
  run grep -qE '\(\(_LOCKED_ACCOUNTS\+\+\)\)' <<< "$block"
  [[ "$status" -ne 0 ]]
}
...
@test "S27: chronyc state marker is char 2 — '^?'/'^x' counted, '^*'/'^+' not" {
  ...
  result=$(_chrony_source_counts <<'EOF'
...
=+ 192.0.2.10                  2   8   377    31    -22us[  -20us] +/-   14ms
...
@test "S27: zero source rows are unassessed, never a vacuous PASS" {
  result=$(_chrony_source_counts <<'EOF'
...
EOF
  ...
  [[ "$result" == "0 0" ]]
  [[ "$(_chrony_source_quality_state 0 0)" == "unassessed" ]]
}

@test "S27: nonempty source quality distinguishes reachable from bad" {
  [[ "$(_chrony_source_quality_state 4 0)" == "pass" ]]
  [[ "$(_chrony_source_quality_state 4 1)" == "warn" ]]
}
...
@test "S27: NoID restricted chronyd is a first-class active NTP service" {
  # Called indirectly through _active_chrony_unit.
  # shellcheck disable=SC2317
  systemctl() {
    [[ "$*" == "is-active --quiet chronyd-restricted.service" ]]
  }
  run _active_chrony_unit
  [[ "$status" -eq 0 ]]
  [[ "$output" == "chronyd-restricted.service" ]]
  grep -q '_CHRONY_ACTIVE_UNIT=.*_active_chrony_unit' "$SCRIPT"
  grep -q '_CHRONY_SECURITY_UNIT=.*_active_chrony_unit' "$SCRIPT"
}
...
@test "S27: every chronyc invocation in main script uses -n (no reverse-DNS)" {
  ...
  # rather than human-readable text that happens to contain "(chronyc ...".
  # Anything after the command-position token not starting with '-' is a
  # violation.
  SCRIPT="${BATS_TEST_DIRNAME}/../../noid-privacy-linux.sh"
  [[ -f "$SCRIPT" ]] || skip "main script not found"
  violations=$(grep -E '(<|\$)\(chronyc[[:space:]]+[^-]' "$SCRIPT" || true)
  [[ -z "$violations" ]]
}

@test "S27: every chronyc invocation closes inherited stdin" {
  # On SELinux desktops an audit launched through SSH otherwise makes
  # chronyc_t try to read an sshd_session_t pipe, creating three AVCs per run.
  SCRIPT="${BATS_TEST_DIRNAME}/../../noid-privacy-linux.sh"
  [[ -f "$SCRIPT" ]] || skip "main script not found"
  invocations=$(grep -E '\(chronyc -n ' "$SCRIPT")
  [[ "$(grep -c . <<< "$invocations")" -eq 2 ]]
  run grep -vF '</dev/null' <<< "$invocations"
  [[ "$status" -ne 0 ]]
}
