...
@test "GDM reads daemon keys with native last-assignment behavior" {
  printf '[daemon]\nAutomaticLoginEnable=false\nAutomaticLoginEnable=true\n' > "$BATS_TEST_TMPDIR/gdm.conf"
  run _dm_section_last_value gdm AutomaticLoginEnable "$BATS_TEST_TMPDIR/gdm.conf"
  [[ "$status" -eq 0 && "$output" == true ]]
}

@test "GDM ignores unrelated groups and differently cased key names" {
  printf '[daemon]\nAutomaticLoginEnable=false\nautomaticloginenable=true\n[debug]\nAutomaticLoginEnable=true\n' > "$BATS_TEST_TMPDIR/gdm.conf"
  run _dm_section_last_value gdm AutomaticLoginEnable "$BATS_TEST_TMPDIR/gdm.conf"
