...
@test "central NSS sources do not suppress local password lifecycle checks" {
  ...
  # The loop must iterate the cached local account database, never getent/NSS.
  [[ "$lifecycle_block" == *'done < <(_passwd_lines)'* ]]
  [[ "$lifecycle_block" != *'getent passwd'* ]]
...
@test "the account database is opened exactly once per audit" {
  # The main-shell loader is the only production path that names the file.
  # Every account check must consume the inherited snapshot instead.
  run grep -cF '_load_passwd_snapshot /etc/passwd' "$SCRIPT"
  [[ "$output" -eq 1 ]]

  run grep -nE 'done < /etc/passwd|(awk|grep)[^|]*[[:space:]]/etc/passwd([[:space:]]|\)|$)' "$SCRIPT"
  [[ "$status" -ne 0 ]]
}

@test "the account snapshot is reused across process substitutions" {
  local account_db="$BATS_TEST_TMPDIR/passwd"
  local -a first=() second=()
  printf '%s\n' 'alice:x:1000:1000:Alice:/home/alice:/bin/bash' > "$account_db"

  _load_passwd_snapshot "$account_db"
  run _passwd_lines
  [[ "$status" -eq 0 ]]
  [[ "$output" == 'alice:x:1000:1000:Alice:/home/alice:/bin/bash' ]]

  # Mutate the source, then mirror the production call shape twice. Both
  # subshells must inherit the first snapshot rather than reopening the file.
  printf '%s\n' 'mallory:x:2000:2000:Mallory:/home/mallory:/bin/bash' > "$account_db"
  mapfile -t first < <(_passwd_lines)
  mapfile -t second < <(_passwd_lines)
  [[ "${first[*]}" == 'alice:x:1000:1000:Alice:/home/alice:/bin/bash' ]]
  [[ "${second[*]}" == 'alice:x:1000:1000:Alice:/home/alice:/bin/bash' ]]
