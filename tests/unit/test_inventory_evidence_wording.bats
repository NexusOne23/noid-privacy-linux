...
@test "unmuted microphone state is not called active recording" {
  grep -q 'Default audio source unmuted' "$SCRIPT"
  grep -q 'does not prove recording' "$SCRIPT"
  run grep -q 'Microphone active for' "$SCRIPT"
  [[ "$status" -ne 0 ]]
}

@test "history finding counts use singular and plural wording" {
  # shellcheck disable=SC2016  # literal production-source output
  grep -Fq '$_SH_SUSP suspicious $(_plural "$_SH_SUSP" entry entries)' "$SCRIPT"
  # shellcheck disable=SC2016  # literal production-source output
  grep -Fq '$sensitive potential $(_plural "$sensitive" secret secrets)' "$SCRIPT"

  # shellcheck disable=SC2016  # literal obsolete production-source output
  run grep -E '\$(_SH_SUSP|sensitive) (suspicious entries|potential secrets)' "$SCRIPT"
  [[ "$status" -ne 0 ]]
}
