...
setup() {
  ...
  # shellcheck disable=SC1090
  source <(awk '/^_aide_print_drift_lines\(\) \{/,/^}/ {print}' "$SCRIPT")
  # shellcheck disable=SC1090
  ...
  # shellcheck disable=SC1090
  source <(awk '/^_iface_link_kind\(\) \{/,/^}/ {print}' "$SCRIPT")
  # shellcheck disable=SC1090
...
@test "AIDE report rendering preserves categories and full paths containing colons" {
  local report="$BATS_TEST_TMPDIR/aide-render.log" result
  printf '%s\n' \
    'Added entries:' 'f+++++++++++++++++: /new: path: with spaces' \
    'Removed entries:' 'f-----------------: /removed' \
    'Changed entries:' 'f = ...   i...... : /changed: suffix' \
    'Detailed information about changes:' > "$report"
  result=$(_aide_report_drift_lines "$report" | _aide_print_drift_lines)
  [[ "$result" == $'       Added  : /new: path: with spaces\n       Removed: /removed\n       Changed: /changed: suffix' ]]
  grep -Fq '_aide_print_drift_lines < <(' "$SCRIPT"
}

@test "AIDE rendering also accepts raw journal diff markers without truncating paths" {
  local result
  result=$(printf '%s\n' \
    'f+++++++++++++++++: /new: path' \
    'f-----------------: /removed: path' \
    'f = ...   i...... : /changed' | _aide_print_drift_lines)
  [[ "$result" == $'       Added  : /new: path\n       Removed: /removed: path\n       Changed: /changed' ]]
}

@test "AIDE drift rendering keeps terminal controls visible instead of executing them" {
  local result
  result=$(printf 'Added  : /new: \033[31mfile\177\n' | _aide_print_drift_lines)
  [[ "$result" == '       Added  : /new: \u001b[31mfile\u007f' ]]
}
...
@test "egress observations stay on explicit IPv4 without logging public addresses" {
  block=$(sed -n '/# Compare direct-DNS and HTTPS egress observations/,/# IPv6 (/p' "$SCRIPT")
  [[ "$block" == *'addresses redacted'* ]]
  [[ "$block" == *'dig -4 +short +time=5 whoami.akamai.net'* ]]
  [[ "$block" == *'curl -4 -fsS --max-time 5 https://ifconfig.me/ip'* ]]
  [[ "$block" != *'_EGRESS_FAMILY_ARGS'* ]]
...
@test "VPN activity requires the administrative UP flag" {
  ...
  grep -Fq '_VPN_LINK_STATE=$(_iface_admin_state "$IFACE")' "$SCRIPT"
...
@test "VPN classification separates actual tunnels from dummy and bridged TAP" {
  _iface_link_kind() {
    case "$1" in
      proton0) printf '%s\n' wireguard ;;
      tap-vm) printf '%s\n' virtual ;;
      tun-nm|tun-route) printf '%s\n' tun ;;
      proton-fake) printf '%s\n' dummy ;;
      *) printf '%s\n' other ;;
    esac
  }
...
@test "root arithmetic inputs from config files are digit-gated" {
  ...
  # SSH numeric producer failures, malformed values and overflow boundaries
  # are exercised through production functions in test_ssh_evidence.bats.
  grep -q 'GPG default-cache-ttl absent or unrecognized' "$SCRIPT"
...
@test "RPM verify is side-effect-safe and baseline writes are atomic" {
  # Literal source-code pattern.
  # shellcheck disable=SC2016
  grep -A1 '_run_timed_capture_all_closed RPM_VA_OUTPUT' "$SCRIPT" | grep -q 'env LC_ALL=C rpm -Va --noscripts'
...
@test "operational findings are not promoted to security FAIL by raw counts" {
  ...
  grep -q 'Module squashfs: present (used by snaps and mounted images' "$SCRIPT"
  # shellcheck disable=SC2016  # literal source-code pattern
