...
@test "RPM fingerprints preserve trailing newlines in symlink targets" {
  local path="$BATS_TEST_TMPDIR/link" first second
  ...
  second=$(printf '%s\n' "$path" | _rpm_baseline_records)
  [[ "$first" != "$second" ]]
  printf '%s\n' "$second" | _rpm_policy_records_valid
}
...
@test "missing NoID image policy cannot be replaced by a generic baseline" {
  grep -q 'NoID RPM reference profile: optional, not configured' "$SCRIPT"
