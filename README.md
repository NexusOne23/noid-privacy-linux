...
[![Version](https://img.shields.io/badge/version-3.7.2-green.svg)](https://github.com/NexusOne23/noid-privacy-linux/releases)
...
**42 risk-weighted sections · Pure Bash · Zero dependencies · Structured remediation prompts**

**Changes nothing by default · Root is used only to read · Your report never leaves the machine**
...
<p align="center">
  <a href="Docs/screenshots/noid-linux-audit-prior-cycle.png">
    <img src="Docs/screenshots/noid-linux-audit-prior-cycle.png" alt="Prior-cycle NoID Privacy for Linux terminal audit on NoID Privacy Workstation 44, showing kernel and boot integrity, SELinux, and firewall posture findings" width="900">
  </a>
</p>

<p align="center"><sub>Prior-cycle validation screenshot on Workstation 44 · the v3.7.2 report format is unchanged · click to enlarge</sub></p>

---

## ⚡ Quick Start

> **Why root?** The audit reads evidence an unprivileged user cannot see — `/etc/shadow` permissions, auditd rules, the live firewall ruleset, LUKS keyslot KDFs, package-verification state. It changes nothing: no configuration, no services, no package metadata, no upload. Every stateful path is opt-in and documented in [Docs/CHECKS.md](Docs/CHECKS.md).

Run from this repository checkout, or download the tagged **3.7.2** release as
shown under [Installation](#-installation) — both carry the same bytes. The
[source validation record](Docs/SUPPORT.md#source-boundary) identifies the
exact audited script.

```bash
printf '%s  %s\n' \
  '30ce5f49a8d119a8801620c342e146da087210d68aa920d06083571f05d7546c' \
  'noid-privacy-linux.sh' | sha256sum --check --strict && \
...
from FAIL/WARN findings, selected INFO evidence limits, and unassessed weighted
sections. Review its suggestions against your distribution's maintained
...
Turn the findings into a ready-to-paste prompt for the assistant of your choice:
...
FAIL/WARN findings, selected INFO evidence limits, unassessed weighted sections,
and sanitized system context. This includes unverified kill-switch protection,
an unestablished NoID AIDE baseline and unclassified NoID RPM changes when
reported. Other INFO findings can still matter; section coverage does not attest
every control. A model can explain findings and propose commands, but its output
is untrusted advice: verify scope, package names, paths, rollback, and
distribution guidance before changing the system.
...
| **App Telemetry** | GNOME telemetry, crash reporters, Flatpak permission grants, DNF countme |
...
Illustrative excerpt — actual findings, score and coverage depend on the host.

```
$ sudo bash noid-privacy-linux.sh --ai

╔══════════════════════════════════════════════════════════════════════╗
║  🛡️  NoID Privacy for Linux · v3.7.2
...
╠══════════════════════════════════════════════════════════════════════╣
║  Scan completed:         100% (42/42 sections)
...
| Flag | Description |
|------|-------------|
| `--ai` | Generate a local prompt with FAIL/WARN, selected INFO evidence limits, and unassessed sections |
...
The commands below retrieve the tagged **v3.7.2** artifact. It carries the same
bytes as this repository checkout, so the Quick Start checksum applies to both.
The published identity is recorded in
[Support and release validation](Docs/SUPPORT.md#published-v372-artifact).

```bash
# Tagged v3.7.2 bytes: download, verify, then run
curl --proto '=https' --tlsv1.2 -fL \
  https://raw.githubusercontent.com/NexusOne23/noid-privacy-linux/v3.7.2/noid-privacy-linux.sh \
  -o noid-privacy-linux.sh
printf '%s  %s\n' \
  '30ce5f49a8d119a8801620c342e146da087210d68aa920d06083571f05d7546c' \
  'noid-privacy-linux.sh' | sha256sum --check --strict && \
  sudo bash noid-privacy-linux.sh --ai

# Or clone that tag and verify the same script bytes
git clone --branch v3.7.2 --depth 1 https://github.com/NexusOne23/noid-privacy-linux.git
...
The release tag resolves to the reviewed release commit, whose full immutable ID
is recorded on the release page. The SHA-256 check detects byte drift before
root execution. Both values come from this repository — they prove
reproducibility, not third-party signing.
...
Pin the action to the reviewed `v3.7.2` release. The release page records the
full immutable commit ID of this tag; substitute it for the tag in workflows
that require a non-movable reference, because a tag can be repointed while a
commit ID cannot.

```yaml
- name: Hardening Posture Audit
  uses: NexusOne23/noid-privacy-linux@v3.7.2
  id: audit
  with:
...
jobs:
  audit:
    ...
    steps:
      - uses: actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1  # v7.0.1 — commit SHA
      - uses: NexusOne23/noid-privacy-linux@v3.7.2
        with:
          min-score: '70'
          min-coverage: '75'
